...
from vinyl_robot_msgs.msg import SafetyStatus
...
class LidarSafetyNode(Node):
    def __init__(self):
        ...
        self._warn_r = (
            self.get_parameter("warning_radius_mm").get_parameter_value().double_value / 1000.0
        )
        self._stop_r = (
            self.get_parameter("stop_radius_mm").get_parameter_value().double_value / 1000.0
        )

        self._a_angle_deg: float = 0.0

        # Single composite publisher replaces separate velocity_scale + estop topics
        ...
        if self._mock:
            ...
            self.create_subscription(UInt8MultiArray, "/canopen/a_axis/tpdo1", self._on_a_tpdo, 10)
            self.get_logger().info("lidar_safety: waiting for /scan from SICK TIM571")

    def _publish_safe(self) -> None:
        self._safety_pub.publish(SafetyStatus(velocity_scale=1.0, estop=False, reason=""))
    ...
    def _on_scan(self, msg: LaserScan) -> None:
        ...
        if min_range <= self._stop_r:
            status = SafetyStatus(velocity_scale=0.0, estop=True, reason="lidar_stop_zone")
            self.get_logger().warn(f"lidar_safety: STOP zone (min_range={min_range:.2f}m)")
        elif min_range <= self._warn_r:
            status = SafetyStatus(velocity_scale=0.25, estop=False, reason="lidar_warning_zone")
        else:
            status = SafetyStatus(velocity_scale=1.0, estop=False, reason="")
